...
class MapNet(nn.Module): 
    ...
    def build_loss(self, p_pred, p_gt):
        ...
        if self.loss_type=="BCE":
            ...
            for i in range(p_gt.shape[0]):
                ind = torch.nonzero(p_gt[i,:]) # label that signifies the ground-truth position and orienation
    ...
    def forward_single_step(self, local_info, t, input_flags, map_previous=None, p_given=None, update_type="lstm"):
        ...
        grid = np.zeros((batch_size, self.map_embedding, self.observation_dim[0], self.observation_dim[1]), dtype=np.float32)
        ...
        for b in range(batch_size):
            ...
            grid_step, map_occ_step = self.groundProjectionStep(img_feat=img_feat_step, points2D=points2D_step, 
                                                local3D=local3D_step, sseg=sseg_step, dets=dets_step, input_flags=input_flags)
            grid[b,:,:,:] = grid_step
    ...
    def forward(self, local_info, update_type, input_flags, p_gt=None):
        ...
        grid, map_occ = self.groundProjection(img_feat_all=img_feat, points2D_all=points2D, local3D_all=local3D, 
                                        sseg_all=sseg, dets_all=dets, batch_size=batch_size, seq_len=seq_len, input_flags=input_flags)
    ...
    def groundProjection(self, img_feat_all, points2D_all, local3D_all, sseg_all, dets_all, batch_size, seq_len, input_flags):
        ...
        grid = np.zeros((batch_size, seq_len, self.map_embedding, self.observation_dim[0], self.observation_dim[1]), dtype=np.float32)
        grid = torch.tensor(grid, dtype=torch.float32).cuda()
        ...
        for b in range(batch_size):
            ...
            for q in range(seq_len):
                ...
                dets_step = dets_all[b,q,:,:,:].unsqueeze(0) # 1 x classes x crop_size(1) x crop_size(0)
                grid_step, map_occ_step = self.groundProjectionStep(img_feat=img_feat_step, points2D=points2D_step, 
                                                        local3D=local3D_step, sseg=sseg_step, dets=dets_step, input_flags=input_flags)
    ...
    def groundProjectionStep(self, img_feat, points2D, local3D, sseg, dets, input_flags):
        ...
        map_coords, valid, map_occ = dh.discretize_coords(x=local3D[:,0], z=local3D[:,2], map_dim=self.observation_dim, cell_size=self.cell_size)
        map_occ = torch.tensor(map_occ, dtype=torch.float32).cuda()
    ...
    def do_rotation(self, grid, angle):
        ...
        grid_affine = F.affine_grid(theta, grid.size())
        return F.grid_sample(grid, grid_affine)
        ...
        if update_type=="lstm":
            ...
            for i in range(self.global_map_dim[0]):
                for j in range(self.global_map_dim[1]):
                    ...
                    hidden = (emb_hidden.contiguous(), emb_hidden.contiguous()) # LSTM expects two hidden inputs
